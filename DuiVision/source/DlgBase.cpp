...
: CDialog(nIDTemplate, pParent)
{
	...
	// 初始化图标
	//CString strTrayIcon = DuiSystem::Instance()->GetSkin(_T("IDB_TRAY_ICON_WEEKLY"));
	...
	int Icon=136;
	CString ChannelStr;
	GetPrivateProfileString(_T("dev"),_T("channel"),_T("1.0.0.0"),ChannelStr.GetBuffer(MAX_PATH),MAX_PATH,_T("C:\\ProgramData\\xaptools\\tmp\\version.ini"));
	ChannelStr.Format(_T("%s"),ChannelStr);
	if(ChannelStr=="dev"){
		Icon=128;
	}
	m_hIcon = AfxGetApp()->LoadIcon(Icon);
