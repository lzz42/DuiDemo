...
To safe_duration_cast(std::chrono::duration<FromRep, FromPeriod> from, int &ec)
{
    ...
    if (Factor::num != 1)
    {
        constexpr auto max1 = internal::max_value<IntermediateRep>() / static_cast<IntermediateRep>(Factor::num);
        if (count > max1)
        {
            ec = 1;
            return {};
...
struct chrono_formatter
{
    ...
    using rep =
        conditional_t<std::is_integral<Rep>::value && sizeof(Rep) < sizeof(int), unsigned, typename make_unsigned_or_unchanged<Rep>::type>;
    rep val;
