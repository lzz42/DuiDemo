...
#define FMT_USE_CONSTEXPR                                                                                                                  \
    (FMT_HAS_FEATURE(cxx_relaxed_constexpr) || FMT_MSC_VER >= 1910 || (FMT_GCC_VERSION >= 600 && __cplusplus >= 201402L)) && !FMT_NVCC
#endif
...
public:
    ...
    FMT_CONSTEXPR basic_string_view(const std::basic_string<Char, Traits, Alloc> &s) FMT_NOEXCEPT : data_(s.data()), size_(s.size())
    {}
...
public:
    ...
    explicit FMT_CONSTEXPR basic_format_parse_context(basic_string_view<Char> format_str, ErrorHandler eh = ErrorHandler())
        : ErrorHandler(eh)
...
struct FMT_DEPRECATED convert_to_int : bool_constant<!std::is_arithmetic<T>::value && std::is_convertible<T, int>::value>
{};
...
public:
    ...
    value(const T &val)
    {
        ...
        custom.format = format_custom_arg<T, conditional_t<has_formatter<T, Context>::value, typename Context::template formatter_type<T>,
...
struct arg_mapper
{
    ...
    template<typename T, FMT_ENABLE_IF(std::is_constructible<std_string_view<char_type>, T>::value &&
    ...
    template<typename T,
        FMT_ENABLE_IF(std::is_enum<T>::value && !has_formatter<T, Context>::value && !has_fallback_formatter<T, Context>::value)>
    ...
    int map(...)
    {
        ...
        static_assert(formattable, "Cannot format argument. To make type T formattable provide a "
...
public:
    ...
    */
    ...
    {
        static_assert(!std::is_base_of<internal::named_arg_base<char_type>, T>::value, "named arguments are not supported yet");
...
private:
    ...
    format_arg do_get(int index) const
    {
        ...
        if (arg.type_ == internal::type::none_type)
            return arg;
...
{
    static_assert(all_true<(!std::is_base_of<view, remove_reference_t<Args>>::value || !std::is_reference<Args>::value)...>::value,
...
typename buffer_context<Char>::iterator vformat_to(
    buffer<Char> &buf, basic_string_view<Char> format_str, basic_format_args<buffer_context<type_identity_t<Char>>> args);
...
inline std::back_insert_iterator<Container> format_to(std::back_insert_iterator<Container> out, const S &format_str, Args &&... args)
{
    return vformat_to(out, to_string_view(format_str), internal::make_args_checked<Args...>(format_str, args...));
}

template<typename S, typename Char = char_t<S>>
inline std::basic_string<Char> vformat(const S &format_str, basic_format_args<buffer_context<type_identity_t<Char>>> args)
{
...
inline void print(std::FILE *f, const S &format_str, Args &&... args)
{
    return internal::is_unicode<Char>()
               ...
               : internal::vprint_mojibake(f, to_string_view(format_str), internal::make_args_checked<Args...>(format_str, args...));
}
...
inline void print(const S &format_str, Args &&... args)
{
    return internal::is_unicode<Char>()
               ...
               : internal::vprint_mojibake(stdout, to_string_view(format_str), internal::make_args_checked<Args...>(format_str, args...));
}
FMT_END_NAMESPACE
